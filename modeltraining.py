...
⚠️  Tips:
  ...
  - Can redo if you mess up (press 'q' and try again)
    ...
    # Save data
    if len(frames_data) >= 50:  # At least 1 second
        ...
        print(f"  ⚠️  Not enough frames ({len(frames_data)}/50). Try again.")
        return False
...
def collect_asl_signs():
    ...
    videos_per_sign = int(input(f"\nHow many videos per sign? (5-50): ") or "10")
    videos_per_sign = max(5, min(50, videos_per_sign))
